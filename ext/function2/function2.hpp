...
// - FU2_HAS_NO_FUNCTIONAL_HEADER
    ...
    (!defined(FU2_HAS_DISABLED_EXCEPTIONS) || defined(FU2_HAS_LIMITED_EMPTY_PROPAGATION))
#include <functional>
#else
	...
	inline namespace abi_400 {
		namespace detail {
			...
			namespace invocation {
				...
				constexpr auto invoke(Callable&& callable, Args&&... args) noexcept(noexcept(std::forward<Callable>(callable)(std::forward<Args>(args)...)))
				...
				constexpr auto invoke(Type T::* member, Self&& self, Args&&... args) noexcept(noexcept((std::forward<Self>(self).*member)(std::forward<Args>(args)...)))
				...
				template<typename T, typename Type, typename Self, typename... Args>
				constexpr auto
				invoke(Type T::* member, Self&& self, Args&&... args) noexcept(noexcept((std::forward<Self>(self)->*member)(std::forward<Args>(args)...)))
				    -> decltype((std::forward<Self>(self)->*member)(std::forward<Args>(args)...)) {
					return (std::forward<Self>(self)->*member)(std::forward<Args>(args)...);
				}
				...
				struct can_invoke<Pointer, identity<T&, Args...>, decltype((void)((std::declval<T&>().*std::declval<Pointer>())(std::declval<Args>()...)))>
				    : std::true_type {};
				template<typename Pointer, typename T, typename... Args>
				struct can_invoke<Pointer, identity<T&&, Args...>, decltype((void)((std::declval<T&&>().*std::declval<Pointer>())(std::declval<Args>()...)))>
				    : std::true_type {};
				...
				struct is_noexcept_correct<true, T, identity<Args...>>
				    : std::integral_constant<bool, noexcept(::fu2::detail::invocation::invoke(std::declval<T>(), std::declval<Args>()...))> {};
			...
			namespace type_erasure {
				...
				struct box : private Allocator {
					...
					explicit box(T value, Allocator allocator_) : Allocator(std::move(allocator_)), value_(std::move(value)) {}
				...
				struct box_factory<box<IsCopyable, T, Allocator>> {
					using real_allocator = typename std::allocator_traits<std::decay_t<Allocator>>::template rebind_alloc<box<IsCopyable, T, Allocator>>;
					...
					static box<IsCopyable, T, Allocator>* box_allocate(box<IsCopyable, T, Allocator> const* me) {
						...
						return static_cast<box<IsCopyable, T, Allocator>*>(std::allocator_traits<real_allocator>::allocate(allocator_, 1U));
					}
				...
				template<bool IsCopyable, typename T, typename Allocator>
				auto make_box(std::integral_constant<bool, IsCopyable>, T&& value, Allocator&& allocator_) {
					return box<IsCopyable, std::decay_t<T>, std::decay_t<Allocator>>(std::forward<T>(value), std::forward<Allocator>(allocator_));
				}
				...
				union data_accessor {
					...
					explicit constexpr data_accessor(void const* ptr) noexcept : data_accessor(const_cast<void*>(ptr)) {}
				...
				using transfer_const_t = std::conditional_t<std::is_const<std::remove_pointer_t<From>>::value, std::add_const_t<To>, To>;
				template<typename From, typename To>
				using transfer_volatile_t = std::conditional_t<std::is_volatile<std::remove_pointer_t<From>>::value, std::add_volatile_t<To>, To>;
	...
	struct function_trait<Ret(Args...) CONST VOLATILE OVL_REF NOEXCEPT> {                                                                                        \
		...
		struct internal_invoker {                                                                                                                                  \
			static Ret invoke(data_accessor CONST VOLATILE* data, std::size_t capacity, Args... args) NOEXCEPT {                                                     \
				...
				return invocation::invoke(static_cast<std::decay_t<decltype(box->value_)> CONST VOLATILE REF>(box->value_), std::forward<Args>(args)...);              \
			}                                                                                                                                                        \
		};                                                                                                                                                         \
                                                                                                                                                               \
		template<typename T>                                                                                                                                       \
...
#undef FU2_DEFINE_FUNCTION_TRAIT
					...
					struct invoke_table<First, Second, Args...> {
						...
						template<typename T, bool IsInplace>
						struct invocation_vtable : public std::tuple<function_pointer_of<First>, function_pointer_of<Second>, function_pointer_of<Args>...> {
						...
						/// The invocation vtable for a present object
						template<typename T>
						struct invocation_view_vtable : public std::tuple<function_pointer_of<First>, function_pointer_of<Second>, function_pointer_of<Args>...> {
						...
						template<bool IsThrowing>
						struct empty_vtable : public std::tuple<function_pointer_of<First>, function_pointer_of<Second>, function_pointer_of<Args>...> {
...
#define FU2_DEFINE_FUNCTION_TRAIT(CONST, VOLATILE, NOEXCEPT, OVL_REF, REF)                                                                                     \
	template<std::size_t Index, typename Function, typename Ret, typename... Args, typename Next, typename... Signatures>                                        \
	...
	protected:                                                                                                                                                   \
		...
		Ret operator()(Args... args) CONST VOLATILE OVL_REF NOEXCEPT {                                                                                             \
			...
			return std::decay_t<decltype(parent->erasure_)>::template invoke<Index>(static_cast<erasure_t CONST VOLATILE REF>(parent->erasure_),                     \
			                                                                        std::forward<Args>(args)...);                                                    \
		}                                                                                                                                                          \
	};                                                                                                                                                           \
	template<std::size_t Index, typename Config, typename Property, typename Ret, typename... Args>                                                              \
	class operator_impl<Index, function<Config, Property>, Ret(Args...) CONST VOLATILE OVL_REF NOEXCEPT> : copyable<!Config::is_owning || Config::is_copyable> { \
		template<std::size_t, typename, typename...>                                                                                                               \
		friend class operator_impl;                                                                                                                                \
                                                                                                                                                               \
	protected:                                                                                                                                                   \
...
#undef FU2_DEFINE_FUNCTION_TRAIT
				...
				namespace tables {
					...
					class vtable<property<IsThrowing, HasStrongExceptGuarantee, FormalArgs...>> {
						using command_function_t = void (*)(vtable* /*this*/,
						                                    opcode /*op*/,
						                                    data_accessor* /*from*/,
						...
						struct trait {
							...
							static void process_cmd(vtable* to_table, opcode op, data_accessor* from, std::size_t from_capacity, data_accessor* to, std::size_t to_capacity) {
								switch (op) {
								case opcode::op_move: {
							...
							template<typename Box>
							static void
							construct(std::true_type /*apply*/, Box&& box, vtable* to_table, data_accessor* to, std::size_t to_capacity) noexcept(HasStrongExceptGuarantee) {
								// Try to allocate the object inplace
							...
							template<typename Box>
							static void construct(std::false_type /*apply*/,
							                      Box&& /*box*/,
							                      vtable* /*to_table*/,
							                      data_accessor* /*to*/,
						...
						/// The command table
						static void
						empty_cmd(vtable* to_table, opcode op, data_accessor* /*from*/, std::size_t /*from_capacity*/, data_accessor* to, std::size_t /*to_capacity*/) {
							switch (op) {
							case opcode::op_move:
							case opcode::op_copy: {
								to_table->set_empty();
					...
					public:
						...
						void move(vtable& to_table, data_accessor* from, std::size_t from_capacity, data_accessor* to, std::size_t to_capacity) noexcept(
						...
						void copy(vtable& to_table, data_accessor const* from, std::size_t from_capacity, data_accessor* to, std::size_t to_capacity) const {
				...
				public:
					...
					FU2_DETAIL_CXX14_CONSTEXPR erasure(erasure<true, OtherConfig, Property> right) noexcept(Property::is_strong_exception_guaranteed) {
						right.vtable_.move(vtable_, right.opaque_ptr(), right.capacity(), this->opaque_ptr(), capacity());
					}

					template<typename T, typename Allocator = std::allocator<std::decay_t<T>>>
					FU2_DETAIL_CXX14_CONSTEXPR erasure(std::false_type /*use_bool_op*/, T&& callable, Allocator&& allocator_ = Allocator{}) {
						vtable_t::init(
						    vtable_,
						    type_erasure::make_box(std::integral_constant<bool, Config::is_copyable>{}, std::forward<T>(callable), std::forward<Allocator>(allocator_)),
						    this->opaque_ptr(),
						    capacity());
					}
					template<typename T, typename Allocator = std::allocator<std::decay_t<T>>>
					FU2_DETAIL_CXX14_CONSTEXPR erasure(std::true_type /*use_bool_op*/, T&& callable, Allocator&& allocator_ = Allocator{}) {
						if (!!callable) {
							vtable_t::init(
							    vtable_,
							    type_erasure::make_box(std::integral_constant<bool, Config::is_copyable>{}, std::forward<T>(callable), std::forward<Allocator>(allocator_)),
					...
					template<typename OtherConfig>
					FU2_DETAIL_CXX14_CONSTEXPR erasure& operator=(erasure<true, OtherConfig, Property> right) noexcept(Property::is_strong_exception_guaranteed) {
						vtable_.weak_destroy(this->opaque_ptr(), capacity());
						right.vtable_.move(vtable_, right.opaque_ptr(), right.capacity(), this->opaque_ptr(), capacity());
						return *this;
					}
					...
					template<typename T, typename Allocator = std::allocator<std::decay_t<T>>>
					void assign(std::false_type /*use_bool_op*/, T&& callable, Allocator&& allocator_ = {}) {
						vtable_.weak_destroy(this->opaque_ptr(), capacity());
						vtable_t::init(
						    vtable_,
						    type_erasure::make_box(std::integral_constant<bool, Config::is_copyable>{}, std::forward<T>(callable), std::forward<Allocator>(allocator_)),
					...
					template<typename T, typename Allocator = std::allocator<std::decay_t<T>>>
					void assign(std::true_type /*use_bool_op*/, T&& callable, Allocator&& allocator_ = {}) {
						if (!!callable) {
							assign(std::false_type{}, std::forward<T>(callable), std::forward<Allocator>(allocator_));
						} else {
							operator=(nullptr);
						}
					}
					...
					static constexpr decltype(auto) invoke(Erasure&& erasure, Args&&... args) {
						...
						return erasure.vtable_.template invoke<Index>(std::forward<Erasure>(erasure).opaque_ptr(), capacity, std::forward<Args>(args)...);
					}
				};
				...
				public:
					...
					constexpr erasure() noexcept : invoke_table_(invoke_table_t::template get_empty_invocation_table<IsThrowing>()), view_(nullptr) {}

					// NOLINTNEXTLINE(cppcoreguidlines-pro-type-member-init)
					constexpr erasure(std::nullptr_t) noexcept : invoke_table_(invoke_table_t::template get_empty_invocation_table<IsThrowing>()), view_(nullptr) {}

					// NOLINTNEXTLINE(cppcoreguidlines-pro-type-member-init)
					constexpr erasure(erasure&& right) noexcept : invoke_table_(right.invoke_table_), view_(right.view_) {}

					constexpr erasure(erasure const& /*right*/) = default;

					template<typename OtherConfig>
					// NOLINTNEXTLINE(cppcoreguidlines-pro-type-member-init)
					constexpr erasure(erasure<false, OtherConfig, property_t> right) noexcept : invoke_table_(right.invoke_table_), view_(right.view_) {}
					...
					constexpr void assign(std::false_type /*use_bool_op*/, T&& callable) {
						invoke_table_ = invoke_table_t::template get_invocation_view_table_of<std::decay_t<T>>();
						view_.assign_ptr(address_taker<std::decay_t<T>>::take(std::forward<T>(callable)));
					}
					template<typename T>
			...
			struct accepts_one : detail::lazy_and< // both are std::integral_constant
			...
			struct accepts_all<T, identity<Signatures...>, void_t<std::enable_if_t<accepts_one<T, Signatures>::value>...>> : std::true_type {};
...
#undef FU2_DEFINE_USE_OP_TRAIT
			...
			struct assert_wrong_copy_assign {
				static_assert(!Config::is_owning || !Config::is_copyable || std::is_copy_constructible<std::decay_t<T>>::value,
			...
			struct assert_no_strong_except_guarantee {
				static_assert(!IsStrongExceptGuaranteed || (std::is_nothrow_move_constructible<T>::value && std::is_nothrow_destructible<T>::value),
			...
			class function<Config, property<IsThrowing, HasStrongExceptGuarantee, Args...>>
			    : type_erasure::invocation_table::operator_impl<0U, function<Config, property<IsThrowing, HasStrongExceptGuarantee, Args...>>, Args...> {
				...
				template<typename T>
				using assert_no_strong_except_guarantee_t = typename assert_no_strong_except_guarantee<HasStrongExceptGuarantee, std::decay_t<T>>::type;
			...
			public:
				...
				FU2_DETAIL_CXX14_CONSTEXPR function(function<RightConfig, property_t> const& right) : erasure_(right.erasure_) {}

				/// Move construction from another function
				template<typename RightConfig, enable_if_copyable_correct_t<Config, RightConfig>* = nullptr, enable_if_owning_correct_t<Config, RightConfig>* = nullptr>
				FU2_DETAIL_CXX14_CONSTEXPR function(function<RightConfig, property_t>&& right) : erasure_(std::move(right.erasure_)) {}
				...
				FU2_DETAIL_CXX14_CONSTEXPR function(T&& callable) : erasure_(use_bool_op<unrefcv_t<T>>{}, std::forward<T>(callable)) {}
				template<typename T,
				         typename Allocator, //
				         enable_if_not_convertible_to_this<T>* = nullptr,
				         enable_if_can_accept_all_t<T>* = nullptr,
				...
				FU2_DETAIL_CXX14_CONSTEXPR function(T&& callable, Allocator&& allocator_) :
				    erasure_(use_bool_op<unrefcv_t<T>>{}, std::forward<T>(callable), std::forward<Allocator>(allocator_)) {}
				...
				void assign(T&& callable, Allocator&& allocator_ = Allocator{}) {
					erasure_.assign(use_bool_op<unrefcv_t<T>>{}, std::forward<T>(callable), std::forward<Allocator>(allocator_));
				}
	...
	using function_base = detail::function<detail::config<IsOwning, IsCopyable, Capacity>, detail::property<IsThrowing, HasStrongExceptGuarantee, Signatures...>>;

	/// An owning copyable function wrapper for arbitrary callable types.
	template<typename... Signatures>
	using function = function_base<true,
	                               true,
	                               capacity_default, //
	                               true,
	                               false,
	...
	template<typename... Signatures>
	using unique_function = function_base<true,
	                                      false,
	                                      capacity_default, //
	                                      true,
	                                      false,
	...
	template<typename... Signatures>
	using function_view = function_base<false,
	                                    true,
	                                    capacity_default, //
	                                    true,
	                                    false,
...
namespace std {
	...
	struct uses_allocator<::fu2::detail::function<Config, Property>, Alloc> : std::true_type {};
} // namespace std
